  ...
  return (
    <div className="flex flex-col sm:flex-row justify-center gap-5 mt-10 px-4 sm:px-8 bg-offWhite dark:bg-background-dark text-navy dark:text-offWhite-dark py-12">
      <div className="flex flex-col justify-center items-center sm:items-start sm:w-1/2">
        <h1 className="font-urbanist text-xl sm:text-2xl lg:text-2xl font-bold  mb-4 text-center sm:text-left">
          {t("JOIN")}
        </h1>
        <p className="text-base sm:text-sm md:text-base lg:text-lg  mb-4 text-center sm:text-left">
          {t("joinOurTeam")}
        </p>
        <NavLink
          to="/contact"
          className="bg-navy text-offWhite px-6 py-2 rounded-full text-sm sm:text-base font-semibold transition duration-300 hover:bg-gradient-to-r hover:from-navy-dark hover:to-seaGreen-dark transform hover:scale-105 w-full sm:w-auto"
        >
          {t("applyNow")}
        </NavLink>
      </div>
